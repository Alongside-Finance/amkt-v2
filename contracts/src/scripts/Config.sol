...
contract InitialBountyHelper {
    ...
    function tokens() public returns (TokenInfo[] memory) {
        ...
        tokens[0] = TokenInfo(BTC, 1e23);
        tokens[1] = TokenInfo(ETH, 1e23);
        tokens[2] = TokenInfo(BNB, 1e22);
        tokens[3] = TokenInfo(SOL, 1e21);
        tokens[4] = TokenInfo(MATIC, 1e20);
        ...
        tokens[12] = TokenInfo(QNT, 1e12);
        tokens[13] = TokenInfo(ARB, 1e11);
        tokens[14] = TokenInfo(MKR, 1e10);
