...
import {MethodologyCouncilReconstitutionTest} from "test/reconstitution/MethodologyCouncil/MethodologyCouncil.t.sol";
...
contract MCReconstitutionStateTest is MethodologyCouncilReconstitutionTest {
    ...
    address[9] tokensKept = [BTC, WSTETH, BNB, SOL, MATIC, LINK, SHIB, AVAX, UNI];
    ...
    function testVirtualUnits() public {
        ...
        assertEq(units[15].token, address(0xFf4927e04c6a01868284F5C3fB9cba7F7ca4aeC0));
    }
    ...
    function testTokensKept() public {
        ...
        for (uint256 i = 0; i < tokensKept.length; i++) {
            ...
            if (tokensKept[i] != WSTETH) {
                ...
                console2.log(tokensKept[i], IERC20(tokensKept[i]).balanceOf(FULFILLER_SAFE));
            }
        }
    }
    ...
    function testTokensAdded() public {
        ...
        for (uint256 i = 0; i < tokensAdded.length; i++) {
            ...
            if (tokensAdded[i] != ASTETH) {
                ...
                console2.log(tokensAdded[i], IERC20(tokensAdded[i]).balanceOf(FULFILLER_SAFE));
            }
        }
    }
    ...
    function testTokensRemoved() public {
        ...
        for (uint256 i = 0; i < tokensRemoved.length; i++) {
            ...
            console2.log(tokensRemoved[i], IERC20(tokensRemoved[i]).balanceOf(FULFILLER_SAFE));
        }
    }

    function testFulfillerSafeBalances() public {
        console2.log(STETH, IERC20(STETH).balanceOf(FULFILLER_SAFE) / (10 ** IERC20(STETH).decimals()));
