...
import {Constants} from "test/reconstitution/102024/Constants.t.sol";
...
contract FulfillerSafeTest is GnosisTest, Constants {
    ...
    function _satisfyFulfillerBalances(address fulfiller, Bounty memory bounty) internal {
        Dealer dealer = new Dealer();
        (, TokenInfo[] memory ins) = quoter.quoteFulfillBounty(bounty, IERC20(AMKT_PROXY).totalSupply());
    ...
    function runFulfillmentBatch(Bounty memory bounty) public returns (bytes memory) {
        ...
        GnosisTransaction[] memory batch = new GnosisTransaction[](ins.length + 1);

        for (uint256 i; i < ins.length; i++) {
            if (ins[i].token == ASTETH) continue;
            batch[i] = GnosisTransaction({
                to: address(ins[i].token),
                data: abi.encodeWithSelector(bytes4(keccak256("approve(address,uint256)")), INVOKEABLE_BOUNTY, ins[i].units)
            });
        }
        batch[ins.length] = GnosisTransaction({
            to: INVOKEABLE_BOUNTY,
            data: abi.encodeWithSelector(
                bytes4(keccak256("fulfillBounty(((address,uint256)[],address,uint256,bytes32),bool)")), bounty, false
            )
        });
...
contract ReconstitutionTest is GnosisTest, Constants {
    ...
    function tokens() internal returns (TokenInfo[] memory) {
        ...
        _tokens[3] = TokenInfo(_21CO_AVAX, _21CO_AVAX_UNITS);
        ...
        _tokens[15] = TokenInfo(_21CO_TON, _21CO_TON_UNITS);

        // NEW TOKEN
        _tokens[16] = TokenInfo(_21CO_LTC, _21CO_LTC_UNITS);
        return _tokens;
    }
    ...
    function postAndFulfillBounty() internal {
        ...
        Bounty memory _bountyToSet =
            Bounty({infos: tokens(), fulfiller: FULFILLER_SAFE, salt: salt, deadline: BOUNTY_DEADLINE});
